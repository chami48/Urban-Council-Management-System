...
// ---------- DELETE booking ----------
const deleteCrematoriumBooking = async (req, res) => {
  try {
    const { id } = req.params;
    const booking = await Crematorium.findByIdAndDelete(id);
    ...
    res.status(200).json({
      success: true,
      message: "Crematorium booking deleted successfully",
      data: booking,
    });
  } catch (err) {
    console.error("deleteCrematoriumBooking error:", err);
