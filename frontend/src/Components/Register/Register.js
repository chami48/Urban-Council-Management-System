...
// optional: import "sweetalert2/dist/sweetalert2.min.css";
...
export default function Register() {
  ...
  // confirm password (front-end only; NEVER sent to API)
  ...
  useEffect(() => {
    const passwordsOk =
      user.password.length >= 8 && confirmPassword.length > 0 && user.password === confirmPassword;
  ...
  const handleSubmit = async (e) => {
    ...
    // final guard (ensures passwords match too)
    if (!canSubmit) {
      // determine a friendly reason if possible
      if (user.password !== confirmPassword) {
        await Swal.fire("Passwords don't match", "Please re-enter the same password.", "error");
      } else {
        await Swal.fire("Check form", "Please fix validation errors.", "warning");
      }
    ...
    try {
      await axios.post("http://localhost:5000/users", {
        ...
        password: user.password,          // ✅ send only the original password
      ...
      await Swal.fire("Registered", "User registered successfully!", "success");
      navigate("/log");
    } catch (err) {
    ...
    <>
      ...
      <div className="registration-container">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <small>Password strength: {pwdStrength}</small>
          ...
          {user.password.length > 0 && (
            <>
              ...
              />
              {confirmPassword.length > 0 && user.password !== confirmPassword && (
