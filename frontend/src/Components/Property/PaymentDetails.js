...
export default function PaymentDetailsPage() {
  ...
  if (!propertyData) return <p className="text-center text-gray-500">Loading property details...</p>;
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-10 px-4">
      <div className="w-full max-w-3xl">
        ...
        <div className="bg-white shadow-lg rounded-2xl p-6 mb-6 border border-gray-200">
          <h3 className="text-xl font-semibold text-indigo-600 mb-4">Property Information</h3>
          <div className="grid grid-cols-2 gap-y-2 text-gray-700">
            <p><b>Property No:</b> {propertyData.propertyNo}</p>
            <p><b>Branch:</b> {propertyData.branch}</p>
            <p><b>Division:</b> {propertyData.division}</p>
            <p><b>Street:</b> {propertyData.street}</p>
          </div>
        </div>
        ...
        {assessmentData ? (
          <div className="bg-white shadow-lg rounded-2xl p-6 mb-6 border border-gray-200">
            <h3 className="text-xl font-semibold text-indigo-600 mb-4">Owner & Assessment Info</h3>
            <div className="grid grid-cols-2 gap-y-2 text-gray-700">
              <p><b>Owner Name:</b> {assessmentData.ownerName}</p>
              <p><b>NIC:</b> {assessmentData.ownerNIC}</p>
              <p><b>Contact:</b> {assessmentData.contactNo}</p>
              <p><b>Property Type:</b> {assessmentData.propertyType}</p>
              <p><b>Annual Value:</b> LKR {assessmentData.appraisedValue.toLocaleString()}</p>
              <p><b>Tax Rate:</b> {assessmentData.taxRate}%</p>
              <p><b>Annual Tax:</b> LKR {annualTax.toLocaleString()}</p>
              <p><b>Quarterly Tax:</b> LKR {quarterlyTax.toLocaleString()}</p>
            </div>
          </div>
        ) : (
