...
export default function PUser({ booking, onDelete }) {
  ...
  // Use the status field directly from backend (Pending/Approved/Rejected)
    ...
    <div className="bg-white border rounded-lg p-6 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          {/* Event Header */}
          <div className="mb-3">
            <h3 className="text-xl font-semibold text-gray-900 mb-1">{eventName || "Untitled Event"}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
            ...
            <div>
              <p className="text-sm text-gray-600">
                <span className="font-medium">Playground:</span> {playgroundType}
              </p>
              <p className="text-sm text-gray-600">
                <span className="font-medium">Attendees:</span> {expectedAttendees}
              </p>
              <p className="text-sm text-gray-600">
          ...
          <div className="flex items-center gap-3 mb-2">
            <span className={`inline-block text-xs font-medium px-3 py-1 rounded-full border ${getStatusColor(displayStatus)}`}>
              {displayStatus}
            </span>
            {createdAt && (
