...
const PaymentForm = ({ payment, onSuccess, onError }) => {
  ...
  // Destructure payment details safely
  const {
    amount = 0,
    applicantName = "",
    paymentType = "",
    ...
    <div className="payment-form-container">
      ...
      {paymentType === "property_tax" && (
        <div className="payment-info">
          ...
          {/* ✅ Display total amount from TaxCalculationSinhala */}
          <p className="total-amount">
            <strong>Total Payable:</strong> Rs. {amount?.toLocaleString() || "0.00"}
          </p>
        </div>
      )}

      {/* =====================
          Shop Rent Payment Section 
      ====================== */}
      {paymentType === "shop_rent" && (
        <div className="payment-info">
          ...
          <p className="total-amount">
            <strong>Total Rent:</strong> Rs. {amount?.toLocaleString() || "0.00"}
          </p>
        </div>
      )}
        ...
        <button disabled={isLoading || !stripe || !elements} className="pay-button">
          <span>
            {isLoading ? (
              ...
              `💳 Pay Rs. ${amount?.toLocaleString() || "0.00"}`
            )}
          </span>
        </button>
        ...
        {message && (
          <div
            className={`payment-message ${
              message.includes("succeeded") ? "success" : "error"
            }`}
          >
            {message}
          </div>
